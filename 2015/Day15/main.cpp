...
Ingredients parse_input(const std::string &filename) {
  ...
  while (getline(file, line)) {
    ...
    std::sregex_iterator iter(properties_str.begin(), properties_str.end(),
                              regex);
...
std::vector<Amounts> generate_combinations(int length, int total) {
  ...
  for (int i = 0; i <= total; ++i) {
    std::vector<Amounts> sub_combinations =
        generate_combinations(length - 1, total - i);
...
int find_best_score(const Ingredients &ingredients) {
  ...
  for (const auto &combination :
       generate_combinations(ingredients.size(), 100)) {
    best_score =
        std::max(best_score, calculate_score(ingredients, combination));
  }
  return best_score;
}
...
int find_best_cookie(const Ingredients &ingredients) {
  ...
  for (const auto &combination :
       generate_combinations(ingredients.size(), 100)) {
    best_score =
        std::max(best_score, calculate_score2(ingredients, combination));
  }
  return best_score;
}
